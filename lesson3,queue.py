...
name_list = ['三','四','五','啦','红','明','强','丽','马','王','赵']
num = 7
...
模拟打印机
    ...
    在3分钟内任意一秒产生一个打印任务的概率是：task/180,随机数模拟，如果生成的随机数是180，就可以认为生成了一个任务
    
    过程：
        1. 创建一个空打印任务队列，每个任务在生成时被赋予一个’时间戳‘
        2. 一个小时中的每一秒（currentSecond）都需要判断：
        是否有新的打印任务生成，如果有，把它加入打印队列：如果打印机空闲并且队列不为空：
            1. 从队列中拿出一个任务交给打印机
            2. 从加入打印机时间 - 加入队列的时间 = 等待时间
            3. 将该任务的等待时间加入到一个列表中，方便后续时候，计算总的学生打印花费的时间
            4. 基于打印的页数的随机数，求出需要多久时间打印
            打印机工作中，那对于打印机而言，就是工作了一秒：对于打印任务而言，它离打印结束又近了一秒
            打印任务完成
